-- =============================================================================
-- WorkOps Phase 11 — Staff transport driver portal MVP
-- =============================================================================
-- Staff trip fields on trips, driver no-trip days, in-app notifications,
-- RPCs for assign / capture / same-day edit.
...
alter table public.trips
  ...
  add column if not exists driver_captured_at timestamptz,
  add column if not exists driver_capture_by uuid references auth.users (id) on delete set null;
...
create index if not exists driver_no_trip_days_org_driver_idx
  on public.driver_no_trip_days (organisation_id, driver_id, trip_date);
...
begin
  ...
  v_assignment_id := public.assign_trip(v_trip_id, p_driver_id, null);
...
-- ---------------------------------------------------------------------------
-- RPC: capture_staff_trip (driver self-capture)
-- ---------------------------------------------------------------------------

create or replace function public.capture_staff_trip(
  p_organisation_id uuid,
  p_planned_start timestamptz,
  p_staff_company public.staff_transport_company,
  p_area_text text,
...
begin
  ...
  v_trip_date := (p_planned_start at time zone 'Africa/Johannesburg')::date;
  if v_trip_date <> (timezone('Africa/Johannesburg', now()))::date then
    raise exception 'Can only capture trips for today';
  end if;
  ...
  if p_opening_km is null or p_closing_km is null or p_closing_km < p_opening_km then
    raise exception 'Invalid odometer readings';
  end if;
  ...
  perform public.assign_trip(v_trip_id, v_driver_id, null);
...
-- ---------------------------------------------------------------------------
-- RPC: update_staff_trip_capture (driver same-day edit)
-- ---------------------------------------------------------------------------

create or replace function public.update_staff_trip_capture(
  p_trip_id uuid,
...
begin
  ...
  if v_trip_date <> (timezone('Africa/Johannesburg', now()))::date then
    raise exception 'Same-day edit only';
  end if;

  if p_opening_km is not null and p_closing_km is not null and p_closing_km < p_opening_km then
    raise exception 'Closing km must be >= opening km';
  end if;

  v_total := public.calc_total_km(
    coalesce(p_opening_km, t.opening_km),
    coalesce(p_closing_km, t.closing_km)
  );
  ...
  set
    ...
    closing_km = coalesce(p_closing_km, closing_km),
    total_km = v_total,
    driver_captured_at = timezone('utc', now()),
    driver_capture_by = auth.uid(),
    status = case
      when coalesce(p_opening_km, opening_km) is not null
        and coalesce(p_closing_km, closing_km) is not null
      then 'completed'::public.trip_status
      else status
    end
  where id = p_trip_id
  returning * into t;
...
-- ---------------------------------------------------------------------------
-- RPC: delete_staff_trip_capture (driver same-day delete)
-- ---------------------------------------------------------------------------

create or replace function public.delete_staff_trip_capture(p_trip_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if t.driver_capture_by is distinct from auth.uid()
     and t.status <> 'assigned' then
    raise exception 'Can only delete own captures';
  end if;
  ...
  v_trip_date := (t.planned_start at time zone 'Africa/Johannesburg')::date;
  if v_trip_date <> (timezone('Africa/Johannesburg', now()))::date then
    raise exception 'Same-day delete only';
  end if;
  ...
  set released_at = timezone('utc', now()), deleted_at = timezone('utc', now())
...
grant execute on function public.capture_staff_trip(uuid, timestamptz, public.staff_transport_company, text, int, numeric, numeric) to authenticated;
grant execute on function public.update_staff_trip_capture(uuid, public.staff_transport_company, timestamptz, text, int, numeric, numeric) to authenticated;
grant execute on function public.delete_staff_trip_capture(uuid) to authenticated;
