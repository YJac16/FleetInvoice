...
1. Add `npm run env:check` (`scripts/check-env.mjs`) — fail on missing required public/app vars; warn on empty service role, MailerSend, Mapbox, Yoco, and localhost `NEXT_PUBLIC_APP_URL`.
...
5. Secrets (`SUPABASE_SERVICE_ROLE_KEY`, MailerSend, Mapbox, Yoco) stay manual paste by the operator — tooling does not invent them.
